...
var station = new Station(randomInt(-200, 250), randomInt(-200, 250), 60, 60); //
...
async function set_move(x, y) {
  move_x = x;
  move_y = y;
  var x_distance = Math.abs(ship.x - x); //dist(ship.x,0,x,0)
  var y_distance = Math.abs(ship.y - y); //dist(0,ship.y,0,y)
